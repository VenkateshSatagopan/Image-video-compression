function Reconstructed_image = IntraDecode2new1(seq, sizes_full, factor,option)
...
while ii <= length(seq)
   ...
   if ii <= sizes_y 
      [dec_output_inst,loc] = ZeroRunDec(seq(ii:end), 1000,64);
       dec_output_Y = [dec_output_Y,dec_output_inst];
       
   elseif ii > sizes_y &&  ii <= (sizes_y+sizes_cb) 

     [dec_output_inst,loc] = ZeroRunDec(seq(ii:end), 1000, 16);
     dec_output_Cb = [dec_output_Cb, dec_output_inst];
     
      elseif  ii > (sizes_y+sizes_cb) && ii <= (sizes_y+sizes_cb+sizes_cr)

    [dec_output_inst,loc] = ZeroRunDec(seq(ii:end), 1000, 16);
    dec_output_Cr = [dec_output_Cr dec_output_inst];
