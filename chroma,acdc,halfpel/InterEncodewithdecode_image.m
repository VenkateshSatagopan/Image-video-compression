function [recon_Y, recon_Cb, recon_Cr,b_rate_curr] = InterEncodewithdecode_image(ORIGINAL_image_Y, ORIGINAL_image_Cb, ORIGINAL_image_Cr, q_factor,M)
...
%%
block_divide=8;
[ac_seq,sizes_full] = IntraEncodeImage_new1(ORIGINAL_image_Y, ORIGINAL_image_Cb, ORIGINAL_image_Cr,q_factor,block_divide);
min_val = -500;
max_val = 1000;
m1=-4;
m2=4;
...
bytestream_dc_new = enc_huffman_new(dc_seq_new-min_val+1, BinCode_dc_new, Codelengths_dc_new);
...
[Reconstructed_imagenew] =IntraDecode2new1(output_ac,sizes_full, q_factor);
recon_Y=Reconstructed_imagenew(:,:,1);
recon_Cb=Reconstructed_imagenew(:,:,2);
recon_Cr=Reconstructed_imagenew(:,:,3);
b_rate_curr = 8*(length(bytestream_ac)+length(bytestream_dc)+length(bytestream_mv))/(size(ORIGINAL_image_Y,1) *size(ORIGINAL_image_Y,2));
end
