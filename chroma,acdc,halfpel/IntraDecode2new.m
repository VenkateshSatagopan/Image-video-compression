function Reconstructed_image = IntraDecode2new(seq, dc_seq,dc_seq_new, sizes_full, factor)
...
len=(dc_size_y+dc_size_cb+dc_size_cr);
dec_dec_new=[];
dec_dec_new=ZeroRunDec(dc_seq_new(1:end), 1000, len);


% DC component Decoding
dec_dc_y_new = Run_DPCM(dc_dec_new(1:dc_size_y), 2);
dec_dc_cb_new = Run_DPCM(dc_dec_new(dc_size_y+1:dc_size_y+dc_size_cb), 2);
dec_dc_cr_new = Run_DPCM(dc_dec_new(dc_size_y+dc_size_cb+1:dc_size_y+dc_size_cb+dc_size_cr), 2);
...
while i <= length(seq)
    ...
    [dec_output_inst,loc] = ZeroRunDec(seq(i:end), 1000, 63);
      dec_output = [dec_output dec_dc(cnt) dec_output_inst];
...
%          t_y=[t_y, dec_output_inst];
%      elseif cnt> dc_size_y && cnt <= 2*dc_size_y
%         t1=2;
%         [dec_output_inst,loc] = ZeroRunDec(seq(i:end), 1000, 63);
%         dec_output_Cb = [dec_output_Cb, dec_dc(cnt), dec_output_inst];
%         t_cb=[t_cb, dec_output_inst];
%     elseif  cnt> 2*dc_size_y && cnt <= 3*dc_size_y;
%         t1=3;
%         [dec_output_inst,loc] = ZeroRunDec(seq(i:end), 1000, 63);
