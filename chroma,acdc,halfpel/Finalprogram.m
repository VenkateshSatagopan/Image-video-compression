...
PSNR(kk,it1) = calcPSNR(8,MSE);
b_rate(kk,it1) = b_rate_curr;
% figure;
...
%err_check=load('D:\IVC\Yogesh\checkresults\error');
%err_im=err_check.err_im;
...
PMF_Motion_Vector = stats_margYCbCr(M, -4, 4);
end
[BinaryTreemv, HuffCodemv, BinCode_mv, Codelengths_mv] = buildHuffman(PMF_Motion_Vector);
...
ref_img(:,:,1)=decoded_frame(:,:,1);
ref_img(:,:,2)=decoded_frame(:,:,2);
ref_img(:,:,3)=decoded_frame(:,:,3);
...
% disp(it1);
% disp('\n');
% psnr_av(it1,name) = mean(PSNR(:,it1))
% b_rate_av(it1,name) = mean(b_rate(:,it1))
% kk=1;
% it1=it1+1;
end
end
